...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const device = await prisma.$transaction(async (tx) => {
      ...
      if (!current) {
        const activeDevices = await tx.licenseDevice.count({ where: { tenantId: tenant.id, deviceRole: parsed.data.deviceRole, revokedAt: null } });
        if (parsed.data.deviceRole === "SERVER" && activeDevices >= 1) throw new Error("SERVER_LIMIT");
        if (parsed.data.deviceRole === "WORKSTATION" && activeDevices >= tenant.licenseSeatLimit) throw new Error("SEAT_LIMIT");
      }
    ...
    if (error instanceof Error && error.message === "SEAT_LIMIT") return NextResponse.json({ error: "已達購買的電腦台數上限，請由管理者撤銷舊裝置或升級方案" }, { status: 409 });
    if (error instanceof Error && error.message === "SERVER_LIMIT") return NextResponse.json({ error: "此公司已有一台本機主機，請由管理者先撤銷舊主機再移機" }, { status: 409 });
