...
export const POST = apiHandler(async (req: NextRequest) => {
  ...
  if (upsert && body.code) {
    const result = await prisma.chartOfAccount.upsert({
      ...
      update: { name: body.name, type: body.type, openingBalance: body.openingBalance, updatedBy: currentUserId },
  ...
  const created = await prisma.chartOfAccount.create({ data: { ...body, tenantId, updatedBy: currentUserId } });
