...
export const GET = apiHandler(async (req: NextRequest) => {
    ...
    prisma.tenant.findMany({
      ...
      select: {
        ...
        users: {
          ...
          take: 1,
          select: { username: true, name: true, email: true, isPaid: true, paymentType: true, subscriptionEnd: true, registrationIp: true },
        },
  ...
  const initializationLogs = tenants.length
  ...
  return NextResponse.json({
    rows: tenants.map((tenant) => {
      ...
      const activeLog = completedLog ?? failedLog ?? startedLog;
      const initializationDetail = parseInitializationDetail(activeLog?.detail ?? null);
      const initializationStatus = completedLog ? "READY" : failedLog && (!startedLog || failedLog.createdAt >= startedLog.createdAt) ? "FAILED" : startedLog ? "RUNNING" : "PENDING";
      ...
      return {
        ...
        owner: owner ? { username: owner.username, name: owner.name, email: owner.email, registrationIp: owner.registrationIp } : null,
        ...
        initialization: {
          ...
          startedAt: typeof initializationDetail.startedAt === "string" ? initializationDetail.startedAt : startedLog?.createdAt.toISOString() ?? null,
          completedAt: typeof initializationDetail.completedAt === "string" ? initializationDetail.completedAt : completedLog?.createdAt.toISOString() ?? null,
          failedAt: typeof initializationDetail.failedAt === "string" ? initializationDetail.failedAt : failedLog?.createdAt.toISOString() ?? null,
