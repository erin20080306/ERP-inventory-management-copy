...
export default function InitializePage() {
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      ...
      const measuredDuration = typeof data.durationMs === "number"
        ? data.durationMs
        : Date.now() - clientStartedAt;
      setDurationMs(measuredDuration);
      setElapsedMs(measuredDuration);
      setStatus("completed");
  ...
  const elapsedSeconds = Math.max(0, (durationMs ?? elapsedMs) / 1000).toFixed(1);
...
function InitializationItem({ icon: Icon, label, done }: { icon: typeof Database; label: string; done: boolean }) {
  return (
