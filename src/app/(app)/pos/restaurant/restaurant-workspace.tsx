...
// 值是 pos 命名空間的鍵；資料庫存的仍是 PAID_IN 等代碼。
...
export function RestaurantWorkspace({ kitchenOnly = false, canManageTables = false }: {
  ...
  const p = useTranslations("pos");
  const tc = useTranslations("common");
  const f = useTranslations("fields");
  const rs = useTranslations("restaurant");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || rs("loadFailed"));
      setData(result);
      if (!kitchenOnly) writeRestaurantBootstrapCache(result);
      setRegisterId((value) => value || result.registers[0]?.id || "");
    } catch (error) {
      if (!cached) toast.error(error instanceof Error ? error.message : rs("loadFailed"));
    } finally {
      setLoading(false);
    }
  ...
  function requestCancelItem(item: OrderItem) {
    ...
    if (requiresWaste && !canCancelWaste) return toast.error(rs("needApproveToWaste"));
    if (!requiresWaste && !canCancelUnprepared) return toast.error(rs("needEditToCancel"));
    setCancelTarget(item);
  }

  async function cancelItem(reason: string, disposition: "NOT_PREPARED" | "WASTE") {
    if (!cancelTarget) return;
    if (disposition === "WASTE" && !canCancelWaste) { toast.error(rs("wasteNeedsApproval")); return; }
    const result = await action({ action: "CANCEL_ITEM", itemId: cancelTarget.id, reason, disposition }, disposition === "WASTE" ? rs("dishCancelledWasted") : rs("dishCancelledAudited"));
  ...
  async function sendKitchen() {
    ...
    const result = await action({ action: "SEND_KITCHEN", orderId: selectedOrder.id }, rs("sentToKitchen"), false);
    if (!result?.ticket) {
      printWindow?.close();
      return;
    }
  ...
  async function requestCashMovement() {
    ...
    if (!Number.isFinite(amount) || amount <= 0 || cashMovementReason.trim().length < 2) return toast.error(p("cashReasonRequired"));
    setBusy(true);
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || p("cashMovementFailed"));
      toast.success(result.message || p("approvalSubmitted"));
      setCashMovementAmount("");
      setCashMovementReason("");
      await load();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : p("cashMovementFailed"));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function decideCashMovement(movementId: string, decision: "APPROVE" | "REJECT") {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || p("supervisorApprovalFailed"));
      toast.success(decision === "APPROVE" ? p("cashMovementApproved") : p("cashMovementRejected"));
      await load();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : p("supervisorApprovalFailed"));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function openShift() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || p("openShiftFailed"));
      toast.success(result.journal ? rs("openShiftWithJournal", { number: result.journal.number }) : rs("shiftOpened"));
      await load();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : p("openShiftFailed"));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function previewCloseShift() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || p("closePreviewFailed"));
      setClosePreview(result.summary);
      setClosingCash("");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : p("closePreviewFailed"));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function closeShift() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || p("closeShiftFailed"));
      toast.success(`${rs("closeShiftDone", { difference: money(Number(result.summary.difference)) })}${result.journal ? rs("closeShiftJournal", { number: result.journal.number }) : ""}`);
      setClosePreview(null);
      setClosingCash("");
      await load();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : p("closeShiftFailed"));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function checkout(method: "CASH" | "CARD", tendered: number, reference?: string) {
    ...
    if (selectedOrder.items.some((item) => item.status === "PENDING")) {
      toast.error(rs("sendAllToKitchenFirst"));
      return;
    }
    if (method === "CASH" && tendered < orderTotal) return toast.error(rs("cashShort"));
    if (method === "CARD" && (!cardApproved || !reference?.trim())) return toast.error(rs("cardApprovalRequired2"));
    setBusy(true);
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(result.error || p("checkoutFailed"));
      }
      ...
      toast.success(rs("paymentDoneToast", { number: result.sale.number }));
      window.setTimeout(() => void load(), 1_200);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : p("checkoutFailed"));
    } finally {
      setBusy(false);
    }
  }
  ...
  if (loading) {
return <div className="grid min-h-[60vh] animate-pulse gap-4 xl:grid-cols-[280px_minmax(0,1fr)_380px]" aria-label={rs("loading")}>
  ...
  if (!data) return <div className="rounded-xl border p-8 text-center">{rs("loadFailed")}</div>;
      ...
      <div>
        <div className="mx-auto mt-10 max-w-xl space-y-6 rounded-3xl border bg-card p-7 shadow-lg">
          <div className="text-center">
            ...
            <h1 className="mt-3 text-2xl font-black">{rs("openShiftTitle")}</h1>
            <p className="mt-2 text-sm text-muted-foreground">{rs("openShiftFirst")}</p>
          </div>
          <div className="flex items-center justify-between rounded-xl bg-muted/40 px-4 py-3 text-sm"><span className="text-muted-foreground">{p("openedBy")}</span><strong>{activeSession?.user?.name || p("currentAccount")}</strong></div>
          ...
          <label className="block text-sm font-medium">
            ...
            <span className="mt-2 block text-xs leading-5 text-muted-foreground">{canManageOpeningCash ? p("openingFloatHint") : p("openingFloatNoPerm")}</span>
          </label>
          <button onClick={openShift} disabled={busy || !registerId} data-shortcut="save" className="h-12 w-full rounded-xl bg-orange-600 font-bold text-white disabled:opacity-50">
            {busy ? rs("processing") : p("confirmOpenShift")}
          </button>
          {allowTableManagement && <button onClick={() => setTableManagerOpen(true)} className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-orange-200 bg-orange-50 text-sm font-bold text-orange-800 dark:border-orange-900 dark:bg-orange-950/30 dark:text-orange-200"><Settings2 className="h-4 w-4" />{rs("noShiftYetTables")}</button>}
        </div>
    ...
    <div className="space-y-4">
        ...
        <DialogContent className="max-w-3xl">
          ...
          <div className="space-y-5">
            <div className="grid gap-3 rounded-xl border bg-muted/30 p-4 md:grid-cols-[180px_160px_1fr_auto]">
              ...
              <input value={cashMovementAmount} onChange={(event) => setCashMovementAmount(event.target.value.replace(/[^\d]/g, ""))} inputMode="numeric" placeholder={p("wholeAmount")} className="h-10 rounded-lg border bg-background px-3 text-right" />
              <input value={cashMovementReason} onChange={(event) => setCashMovementReason(event.target.value)} placeholder={p("cashReasonPlaceholder")} className="h-10 min-w-0 rounded-lg border bg-background px-3" />
              <button onClick={() => void requestCashMovement()} disabled={busy} className="h-10 rounded-lg bg-orange-600 px-4 font-semibold text-white disabled:opacity-40">{p("submitRequest")}</button>
            </div>
            <div className="max-h-[45vh] overflow-auto rounded-xl border">
              <table className="w-full min-w-[760px] text-sm"><thead className="sticky top-0 bg-muted"><tr><th className="p-3 text-left">{p("colTime")}</th><th className="p-3 text-left">{f("type")}</th><th className="p-3 text-right">{tc("amount")}</th><th className="p-3 text-left">{f("reason")}</th><th className="p-3 text-left">{tc("status")}</th><th className="p-3 text-right">{p("supervisorActions")}</th></tr></thead><tbody>
                {data.cashMovements.map((movement) => <tr key={movement.id} className="border-t"><td className="p-3">{new Date(movement.requestedAt).toLocaleString("zh-TW")}</td><td className="p-3">{p(CASH_MOVEMENT_LABEL_KEYS[movement.type])}</td><td className="p-3 text-right font-semibold">{money(Number(movement.amount))}</td><td className="p-3">{movement.reason}</td><td className="p-3">{p(OPERATION_STATUS_LABEL_KEYS[movement.status]) || movement.status}</td><td className="p-3 text-right">{movement.status === "PENDING" && canManageOpeningCash ? <div className="inline-flex gap-2"><button onClick={() => void decideCashMovement(movement.id, "REJECT")} disabled={busy} className="h-8 rounded-lg border px-3">{p("reject")}</button><button onClick={() => void decideCashMovement(movement.id, "APPROVE")} disabled={busy} className="inline-flex h-8 items-center gap-1 rounded-lg bg-emerald-600 px-3 text-white"><ShieldCheck className="h-4 w-4" />{tc("approve")}</button></div> : <span className="text-xs text-muted-foreground">{movement.status === "PENDING" ? p("awaitingSupervisor") : "—"}</span>}</td></tr>)}
                {data.cashMovements.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">{p("noCashMovements")}</td></tr>}
              </tbody></table>
            </div>
          </div>
        </DialogContent>
      ...
      <Dialog open={closePreview !== null} onOpenChange={(open) => { if (!open && !busy) setClosePreview(null); }}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{rs("closeShiftTitle")}</DialogTitle><DialogDescription>{rs("closeShiftNote")}</DialogDescription></DialogHeader>
          {closePreview && <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 rounded-xl border p-4 text-sm"><div><div className="text-muted-foreground">{p("openedBy")}</div><div className="mt-1 font-bold">{closePreview.openedBy.name}</div></div><div><div className="text-muted-foreground">{p("closedBy")}</div><div className="mt-1 font-bold">{closePreview.closingBy.name}</div></div></div>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="rounded-xl bg-orange-50 p-3"><div className="text-orange-700">{rs("shiftNetSales")}</div><div className="mt-1 font-black text-orange-900">{money(closePreview.netSales)}</div></div>
              <div className="rounded-xl bg-emerald-50 p-3"><div className="text-emerald-700">{p("expectedCash")}</div><div className="mt-1 font-black text-emerald-900">{money(closePreview.expectedCash)}</div></div>
            </div>
            <label className="block text-sm font-bold">{p("countedCash")}<input autoFocus value={closingCash} onChange={(event) => setClosingCash(event.target.value.replace(/[^\d]/g, ""))} inputMode="numeric" className="mt-2 h-12 w-full rounded-xl border bg-background px-3 text-right text-xl font-black" /></label>
            <div className="flex items-center justify-between rounded-xl border p-4"><span className="text-sm text-muted-foreground">{p("projectedCashDiff")}</span><strong className={closingCash !== "" && Number(closingCash) === closePreview.expectedCash ? "text-emerald-700" : "text-rose-700"}>{closingCash === "" ? "—" : money(Number(closingCash) - closePreview.expectedCash)}</strong></div>
            <button disabled={busy || closingCash === ""} onClick={() => void closeShift()} className="h-12 w-full rounded-xl bg-orange-600 font-bold text-white disabled:opacity-40">{rs("confirmCloseAndReturnFloat")}</button>
          </div>}
        </DialogContent>
      </Dialog>
      ...
      <div className="grid gap-4 xl:grid-cols-[220px_minmax(0,1fr)_350px]">
        ...
        <section className="rounded-2xl border bg-card p-4">
          <label className="relative block">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={rs("searchDish")} className="h-10 w-full rounded-lg border bg-background pl-9 pr-3" />
          </label>
            ...
            <div className="flex min-h-[540px] flex-col">
              ...
              <div className="flex-1 space-y-2 overflow-y-auto py-3">
                {selectedOrder.items.map((item) => <div key={item.id} className="rounded-xl border p-3">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    {item.status !== "CANCELLED" && ((["PENDING", "SENT"].includes(item.status) && canCancelUnprepared) || (!["PENDING", "SENT"].includes(item.status) && canCancelWaste)) && <button disabled={busy} onClick={() => requestCancelItem(item)} title={rs("cancelDishWithReason")} className="ml-auto h-8 rounded-lg border border-rose-200 px-2 text-xs font-bold text-rose-700 hover:bg-rose-50"><Trash2 className="mr-1 inline h-3 w-3" />{tc("cancel")}</button>}
                  </div>
                </div>)}
              </div>
              <div className="space-y-3 border-t pt-3">
                ...
                <div className="space-y-2 rounded-xl border bg-muted/30 p-3">
                  ...
                  {(invoiceMode === "MOBILE_CARRIER" || invoiceMode === "CITIZEN_CERT") && <input value={invoiceCarrierId} onChange={(event) => setInvoiceCarrierId(event.target.value.toUpperCase())} placeholder={invoiceMode === "MOBILE_CARRIER" ? p("mobileBarcodePlaceholder") : p("citizenCertPlaceholder")} className="h-9 w-full rounded-lg border bg-background px-3 text-sm font-mono uppercase" />}
                  {invoiceMode === "DONATION" && <input value={invoiceDonationCode} onChange={(event) => setInvoiceDonationCode(event.target.value.replace(/\D/g, ""))} placeholder={p("donationCodePlaceholder")} inputMode="numeric" className="h-9 w-full rounded-lg border bg-background px-3 text-sm" />}
                  {invoiceMode === "BUSINESS" && <input value={invoiceBuyerTaxId} onChange={(event) => setInvoiceBuyerTaxId(event.target.value.replace(/\D/g, "").slice(0, 8))} placeholder={p("buyerTaxIdPlaceholder")} inputMode="numeric" className="h-9 w-full rounded-lg border bg-background px-3 text-sm" />}
                  {invoiceMode !== "NONE" && <div className="text-[10px] leading-relaxed text-muted-foreground">{rs("eInvoiceDisclaimer")}</div>}
                </div>
...
function RestaurantRefundHistory({ shift, canRefund, onRefunded }: {
  ...
  const loadSales = useCallback(async () => {
    ...
    const result = await response.json();
    if (!response.ok) throw new Error(result.error || rs("txnLookupFailed"));
  ...
  async function openRefund(saleId: string) {
    if (!shift) return toast.error(p("refundNeedsShift"));
    if (!canRefund) return toast.error(rs("needPosReturnRightsToRefund"));
    setBusy(true);
    try {
      const response = await fetch(`/api/pos/sales/${saleId}`, { cache: "no-store" });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || p("originalTxnLoadFailed"));
  ...
  async function submitRefund() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || p("refundFailed"));
      toast.success(result.message || p("refundDone"));
      setRefundSale(null);
  ...
  return <section id="restaurant-refunds" className="overflow-hidden rounded-2xl border bg-card shadow-sm">
    <div className="flex flex-col justify-between gap-3 border-b p-4 md:flex-row md:items-center"><div><div className="flex items-center gap-2 font-bold"><ReceiptText className="h-5 w-5" />{rs("historyAndRefunds")}</div><div className="mt-1 text-xs text-muted-foreground">{rs("refundNote")}</div></div><div className="flex gap-2"><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void loadSales(); }} placeholder={rs("searchTxn")} className="h-10 min-w-0 rounded-lg border bg-background px-3 text-sm" /><button onClick={() => void loadSales()} className="h-10 rounded-lg border px-4 text-sm">{p("lookup")}</button></div></div>
    <div className="overflow-x-auto"><table className="w-full min-w-[780px] text-sm"><thead className="bg-muted/50 text-xs"><tr><th className="p-3 text-left">{rs("txnTable")}</th><th className="p-3 text-left">{p("colTime")}</th><th className="p-3 text-right">{p("originalAmount")}</th><th className="p-3 text-right">{p("refunded")}</th><th className="p-3 text-left">{tc("status")}</th><th className="p-3 text-right">{tc("actions")}</th></tr></thead><tbody>{sales.map((sale) => {
...
function TableManager({ open, onOpenChange, areas, busy, onAction }: {
  ...
  async function saveTable(event: React.FormEvent) {
    ...
    const result = await onAction({
      ...
      ...form,
    }, editingId ? rs("tableUpdated") : rs("tableAdded"));
    if (result) startCreate();
  }

  async function toggleTable(table: ManagedTable) {
    await onAction({ action: "SET_TABLE_ACTIVE", tableId: table.id, isActive: !table.isActive }, table.isActive ? rs("tableDisabled") : rs("tableRestored"));
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl">
        ...
        <form onSubmit={saveTable} className="grid gap-3 rounded-xl border bg-muted/20 p-4 md:grid-cols-6">
          <label className="text-xs font-bold md:col-span-2">{rs("diningArea")}<select required value={form.areaId} onChange={(event) => setForm({ ...form, areaId: event.target.value })} className="mt-1 h-10 w-full rounded-lg border bg-background px-3 text-sm">{areas.filter((area) => area.isActive).map((area) => <option key={area.id} value={area.id}>{area.name}</option>)}</select></label>
          <label className="text-xs font-bold">{rs("tableCode")}<input required maxLength={20} value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g, "") })} className="mt-1 h-10 w-full rounded-lg border bg-background px-3 font-mono text-sm uppercase" /></label>
        ...
        <div className="max-h-[48vh] space-y-5 overflow-y-auto pr-1">
          ...
          {areas.length === 0 && <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">{rs("noDiningAreas")}</div>}
        </div>
      </DialogContent>
    </Dialog>
  );
...
function KitchenBoard({ tickets, busy, history, canCancelUnprepared, canCancelWaste, refresh, setHistory, cancel, update }: {
  ...
  const statusLabel = (status: string) => status === "SENT" ? rs("waitingToCook") : status === "PREPARING" ? rs("cooking") : status === "READY" ? rs("readyToServe") : status === "SERVED" ? rs("served") : status === "CANCELLED" ? p("statusCancelled") : status;
  ...
  return <div className="space-y-4">
    <header className="flex flex-col justify-between gap-3 rounded-2xl bg-slate-950 p-5 text-white md:flex-row md:items-center">
      <div><h1 className="flex items-center gap-2 text-2xl font-black"><ChefHat className="h-6 w-6 text-orange-400" />{rs("kitchenBoardTitle")}</h1><p className="mt-1 text-xs text-slate-400">{rs("kitchenRefreshNote")}</p></div>
      <div className="flex flex-wrap gap-2"><button onClick={() => setHistory(false)} className={`h-10 rounded-lg px-4 text-sm font-bold ${!history ? "bg-orange-500 text-white" : "border border-white/20 text-white"}`}>{rs("inProgress")}</button><button onClick={() => setHistory(true)} className={`h-10 rounded-lg px-4 text-sm font-bold ${history ? "bg-indigo-500 text-white" : "border border-white/20 text-white"}`}>{rs("todayHistory")}</button><Link href="/pos/restaurant" className="h-10 rounded-lg border border-white/20 px-4 py-2 text-sm">{rs("backToOrdering")}</Link><button onClick={() => void refresh()} className="inline-flex h-10 items-center gap-2 rounded-lg bg-white px-4 text-sm font-bold text-slate-950"><RefreshCw className="h-4 w-4" />{rs("refresh")}</button></div>
    </header>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {tickets.map((ticket) => <article key={ticket.id} className={`rounded-2xl border-2 p-4 shadow-md transition-all ${ticket.status === "SERVED" ? "border-indigo-400 bg-indigo-50/70" : ticket.status === "READY" ? "border-emerald-500 bg-emerald-50/60 ring-2 ring-emerald-200" : ticket.status === "PREPARING" ? "border-orange-500 bg-orange-50/60" : "border-slate-300 bg-card"}`}>
        <div className="flex justify-between gap-3"><div><div className="text-xl font-black">{ticket.order.table.name}</div><div className="text-xs text-muted-foreground">{ticket.number}</div></div><div className="flex items-start gap-2"><button onClick={() => window.open(`/print/kitchen/${ticket.id}`, "_blank", "noopener,noreferrer")} title={rs("printKitchenTicket")} className="rounded-lg border bg-white p-2"><Printer className="h-4 w-4" /></button><span className={`rounded-full px-3 py-1 text-xs font-black ${ticket.status === "SERVED" ? "bg-indigo-600 text-white" : ticket.status === "READY" ? "bg-emerald-600 text-white" : ticket.status === "PREPARING" ? "bg-orange-500 text-white" : "bg-slate-200 text-slate-800"}`}>{statusLabel(ticket.status)}</span></div></div>
        <div className="mt-3 grid grid-cols-2 gap-2 rounded-xl border bg-white/80 p-3 text-xs">
          <div><span className="text-muted-foreground">{rs("sendToKitchen")}</span><strong className="ml-2">{clock(ticket.sentAt)}</strong></div>
      ...
      {tickets.length === 0 && <div className="col-span-full rounded-2xl border border-dashed p-16 text-center text-muted-foreground"><ChefHat className="mx-auto h-12 w-12 opacity-30" /><div className="mt-3 font-bold">{history ? rs("noHistoryToday") : rs("noPendingDishes")}</div></div>}
    </div>
  </div>;
}
