...
export function TrialGate({ children, initialAccess, iosApp = false }: { children: React.ReactNode; initialAccess: LicenseAccess; iosApp?: boolean }) {
  ...
  if (!access.allowed) {
    return <BlockedAccess access={access} checking={checking} onRefresh={refresh} iosApp={iosApp} />;
  }

  return (
    <>
      {access.status === "trial" && <TrialBanner remainMs={access.remainMs ?? 0} iosApp={iosApp} />}
      {access.status === "paid" && access.subscriptionRemainMs !== undefined && access.subscriptionRemainMs <= 7 * 86_400_000 && (
        <div className="sticky top-0 z-[999] flex items-center justify-center gap-2 bg-indigo-600 px-4 py-2 text-sm text-white shadow print:hidden">
          ...
          {!iosApp && <Link href="/plans" className="ml-2 rounded-full bg-white px-3 py-1 text-xs font-semibold text-indigo-700">聯絡續約</Link>}
        </div>
      )}
      {children}
    </>
...
function TrialBanner({ remainMs, iosApp }: { remainMs: number; iosApp: boolean }) {
  ...
  return (
    <div className="sticky top-0 z-[999] flex flex-wrap items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-2 text-sm font-medium text-white shadow print:hidden">
      ...
      {!iosApp && <Link href="/plans" className="ml-2 rounded-full bg-white px-3 py-1 text-xs font-bold text-orange-700">查看方案並聯絡開通</Link>}
    </div>
  );
}

function BlockedAccess({ access, checking, onRefresh, iosApp }: { access: LicenseAccess; checking: boolean; onRefresh: () => Promise<void>; iosApp: boolean }) {
  return (
    <main className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-950 p-6 text-white">
      <section className="w-full max-w-xl rounded-3xl border border-white/10 bg-white/5 p-8 text-center shadow-2xl backdrop-blur-xl">
        ...
        <div className={`mt-7 grid gap-3 ${iosApp ? "" : "sm:grid-cols-2"}`}>
          {!iosApp && <Link href="/plans" className="rounded-xl bg-indigo-500 px-4 py-3 text-sm font-semibold hover:bg-indigo-400">聯絡艾琳設計開通</Link>}
