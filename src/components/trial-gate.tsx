...
type GateState = "checking" | "trial" | "expired" | "paid" | "locked";

const GATE_CACHE_KEY = "erp_trial_gate_state";

function getCachedGateState(): GateState {
  if (typeof window === "undefined") return "checking";
  try {
    const cached = sessionStorage.getItem(GATE_CACHE_KEY);
    if (cached === "paid" || cached === "trial") return cached;
  } catch {}
  return "checking";
}

function rememberGateState(state: GateState) {
  if (typeof window === "undefined") return;
  try {
    if (state === "paid" || state === "trial") sessionStorage.setItem(GATE_CACHE_KEY, state);
    else sessionStorage.removeItem(GATE_CACHE_KEY);
  } catch {}
}

export function TrialGate({ children }: { children: React.ReactNode }) {
  const [state, setState] = useState<GateState>(() => getCachedGateState());
  ...
  const checkTrial = useCallback(async () => {
    try {
      ...
      if (data.status === "paid") {
        ...
        setState("locked");
        rememberGateState("locked");
      } else if (data.status === "expired") {
        setState("expired");
        rememberGateState("expired");
      } else {
        const remain = data.remainMs ?? 0;
        expireAtRef.current = Date.now() + remain;
        setRemainMs(remain);
        setState("trial");
        rememberGateState("trial");
        if (typeof data.tenantCount === "number") setTenantCount(data.tenantCount);
      }
    } catch {
      setState("trial");
        ...
        {checking ? (
          ...
          <div className="rounded-2xl bg-white/5 backdrop-blur-2xl border border-white/10 shadow-2xl p-8 space-y-6">
            ...
            <div className="border-t border-white/10 pt-4 text-xs text-slate-500 space-y-1">
              ...
              <a href="https://erin.is-a.dev/" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300 transition">
      ...
      <div className="max-w-2xl mx-auto space-y-6 py-8">
        ...
        <div className="rounded-2xl bg-white/5 backdrop-blur-2xl border border-white/10 shadow-2xl p-8 space-y-6">
          ...
          <div className="border-t border-white/10 pt-4 text-sm text-slate-400 space-y-1 text-center">
            ...
            <div>Email: erin20080306@gmail.com</div>
            <div>Line ID: erin2008</div>
            <a href="https://erin.is-a.dev/" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300 transition block">
              https://erin.is-a.dev/
            </a>
          </div>
        </div>
