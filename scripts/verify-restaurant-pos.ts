...
async function main() {
  ...
  try {
    ...
    const createdTable = await createRestaurantTable(tenant.id, { areaId: area.id, code: "t13", name: "十三號桌", seats: 6, sortOrder: 13 });
    assert.equal(createdTable.code, "T13");
    const updatedTable = await updateRestaurantTable(tenant.id, createdTable.id, { areaId: area.id, code: "T13", name: "包廂十三號桌", seats: 8, sortOrder: 2 });
    assert.equal(updatedTable.name, "包廂十三號桌");
    ...
    const category = await prisma.productCategory.create({ data: { tenantId: tenant.id, code: `MEAL-${suffix}`, name: "測試主餐" } });
    const unit = await prisma.productUnit.create({ data: { tenantId: tenant.id, code: `PLATE-${suffix}`, name: "份" } });
    const product = await prisma.product.create({ data: { tenantId: tenant.id, sku: `FOOD01-${suffix}`, name: "測試餐點", categoryId: category.id, unitId: unit.id, salePrice: 200, costPrice: 80, imageUrl: "/demo-products/burger.svg" } });
