...
const dashboard = renderZh(readFileSync("src/app/(app)/dashboard/page.tsx", "utf8"));
...
assert.match(restaurant, /公司會計總帳/);
assert.match(restaurant, /跨系統會計數字，不計入本餐飲錢櫃/);
assert.match(restaurant, /開班人員/);
assert.match(restaurant, /結班人員/);
assert.match(dashboard, /const canCash = hasPermission\(permissions, "cash\.view"\)/);
assert.match(dashboard, /canCash && <SummaryCard icon={Banknote} label={庫存現金}/);
