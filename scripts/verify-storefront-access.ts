...
const centralMemberSyncApi = readFileSync("src/app/api/license/storefront-members/route.ts", "utf8");
const localMemberSync = readFileSync("src/lib/storefront-member-sync.ts", "utf8");
...
assert.match(memberRegisterApi, /tx\.customer\.create/);
...
assert.match(centralMemberSyncApi, /ERIN_ERP_STOREFRONT_MEMBERS_V1/);
assert.match(centralMemberSyncApi, /deviceRole !== "SERVER"/);
assert.match(centralMemberSyncApi, /storefrontMember: \{ isNot: null \}/);
assert.match(localMemberSync, /CENTRAL-MEMBER/);
assert.match(localMemberSync, /syncCentralStorefrontMembers/);
assert.match(localMemberSync, /CENTRAL_STOREFRONT_MEMBER_SYNC/);
assert.match(customerApi, /await syncCentralStorefrontMembers\(tenantId\)/);
assert.match(customerApi, /"Cache-Control": "no-store, max-age=0"/);
assert.match(crudTable, /data\?\.warning/);
