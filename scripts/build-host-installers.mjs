...
const imageRepository = "erin-erp-host-bundle";
...
const amd64ImageTag = process.env.ERIN_HOST_IMAGE_AMD64_TAG || `${imageRepository}:${releaseSha}-amd64`;
const arm64ImageTag = process.env.ERIN_HOST_IMAGE_ARM64_TAG || `${imageRepository}:${releaseSha}-arm64`;
...
install_workstation_app() {`,
  ...
  result = replaceOnce(result, "ensure_docker\n\nread -r -p", "ensure_docker\nverify_bundle_architecture\n\nread -r -p", "macOS architecture check");
  result = replaceOnce(result, "pull_erp_image\ndocker_cli compose", "load_bundled_erp_image\ndocker_cli compose", "macOS bundled image call");
  return result;
}
...
Write-Host "艾琳 ERP 公司主機 Windows 安裝程式"`,
  ...
  result = replaceOnce(result, '$ActivationKey = Read-Host', 'Assert-BundleArchitecture\n\n$ActivationKey = Read-Host', "Windows architecture check");
  result = replaceOnce(result, "ERP_IMAGE=ghcr.io/erin20080306/erp-inventory-management-copy:$ImageTag", "ERP_IMAGE=$ErpImage", "Windows bundled image env");
  result = replaceOnce(
    result,
...
2. ${architecture === "amd64" && label.startsWith("Windows") ? "雙擊「安裝艾琳ERP.bat」" : "雙擊「安裝艾琳ERP.command」"}。
...
function prepare({ id, label, architecture, imageArchive, imageTag, fileName }) {
  ...
  mkdirSync(path.join(target, "images"), { recursive: true });
  ...
  writeFileSync(
    ...
    `${imageSha256(imageArchive)}  images/erin-erp-host-image.tar.gz\n`,
  );
  return target;
}
...
const artifacts = packageDefinitions.map((definition) => {
  ...
  if (definition.id === "windows-x64") {
    ...
    writeFileSync(path.join(staging, "安裝艾琳ERP.bat"), rootLauncher);
    writeFileSync(path.join(staging, "installer", "Install-ErinERP.bat"), nestedLauncher);
  } else {
    const transformed = transformMacInstaller(readFileSync(path.join(root, "installer", "安裝艾琳ERP.command"), "utf8"));
    ...
    writeFileSync(path.join(staging, "安裝艾琳ERP.command"), macRootLauncher);
